...
export default function GOLDCOPDPage() {
  ...
  return (
    <CalculatorLayout slug={toolDef.slug} title={toolDef.name} titleEn={toolDef.nameEn} description={toolDef.description}
      ...
      result={<ResultCard label="GOLD分類" value={`GOLD ${spiro} / ${group}群`} interpretation={group === 'A' ? 'LAMA or LABA単剤（参考情報。処方は担当医が決定）' : group === 'B' ? 'LAMA+LABA併用（参考情報。処方は担当医が決定）' : 'LAMA+LABA±ICS（好酸球≧300ならICS追加）（参考情報。処方は担当医が決定）'} severity={sev} />}
