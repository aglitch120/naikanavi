...
export default function PECARNPage(){
  ...
  const result=hasHigh?{sev:'dn' as const,label:'高リスク: 頭部CT推奨（ciTBI 4.3%）'}:hasMed?{sev:'wn' as const,label:'中リスク: CT vs 経過観察を臨床判断（ciTBI <1%）。4-6h観察 or CT検討'}:{sev:'ok' as const,label:'低リスク: CT不要（ciTBI <0.02%）。帰宅指導'}
  return(
