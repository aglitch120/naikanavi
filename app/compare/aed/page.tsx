...
const data: CompareData = {
  ...
  seoContent: [
    ...
    { heading: '妊娠可能女性への配慮', text: 'バルプロ酸は催奇形性リスク（神経管閉鎖障害等）が最も高く、妊娠可能女性には原則使用を避けます。ラモトリギン・レベチラセタムは催奇形性リスクが比較的低いとされ、妊娠を考慮する女性に推奨されます。いずれも葉酸補充（妊娠前から）が重要です。' },
    { heading: 'TDM（治療薬物モニタリング）', text: 'フェニトイン（非線形動態）、バルプロ酸、カルバマゼピンはTDMが推奨されます。フェニトインは低アルブミン血症・腎不全時に遊離型が上昇するため、総濃度の補正（Sheiner-Tozer式）または遊離型測定が必要です。レベチラセタム・ラコサミドは通常TDM不要です。' },
  ],
  references: [
    '各薬剤の添付文書（最新版）',
