...
- Study: コミュニティデッキ + AI生成 + 詳細統計 + ゲーミフィケーション + ストリーク全国ランキング + ストリーク凍結権
...
| Study | デッキで学習 + ストリーク蓄積 | 人気デッキランキング + 学習者数 + 平均正答率 + ストリーク全国ランキング + 自分の順位 |
| 学会カレンダー | 「参加予定」登録 | 参加予定者数の表示 |

---

## オンボーディングフロー（初回訪問 → 活性化）

初回訪問から5秒以内に最初のツールに触れさせる。

1. ホーム画面 → 「あなたは？」モーダル（医学生/初期研修医/専攻医/勤務医）→ 1タップ
2. 選択に応じたおすすめツール3つをハイライト表示
3. 1つ使ってみる → 使用後「お気に入りに追加」パルス
4. Studyへの導線 → 5枚めくる → 「進捗を保存するにはアカウント作成」
5. アカウント作成 → 立場に合ったホーム画面パーソナライズ完了

**コミットメントの積み重ね設計:**
- 立場選択（小さなYes）→ お気に入り追加（中くらいのYes）→ アカウント作成（大きなYes）→ PRO購入（最大のYes）
- 各ステップで「自分用にカスタマイズした」IKEA効果が蓄積

## セッション体験設計（Peak-End Rule）

**ピーク**: 初回ツール利用時のスムーズさ + Studyの3D flipアニメーション
**エンド**: Study終了時に「今日のまとめカード」表示 — ○枚復習、正答率○%、ストリーク○日目（Duolingoモデル）

## 未完了ウィジェット（Zeigarnik Effect）

ホーム画面に常時表示する未完了タスクウィジェット:
- Study: 「今日の残り復習カード: ○枚」
- J-OSLER: 「未提出の症例: ○件」
- EPOC: 「未達成の必須項目: ○件」
- 未完了の数字が目に入るだけで「やらなきゃ」衝動を生む
...
| Study ストリーク7日達成 | 「全国○位！ランキングを見る」PROモーダル | 1回のみ |
