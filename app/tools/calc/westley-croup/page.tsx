...
export default function WestleyPage(){
  ...
  const result=useMemo(()=>{
    ...
    if(score>=8) return {score,severity:'dn' as const,label:'重症（≧8）: 気管挿管を含めた気道管理を専門医と検討 + アドレナリン吸入を検討 + ステロイド投与（用量はガイドライン参照。担当医が決定）'}
    if(score>=3) return {score,severity:'wn' as const,label:'中等症（3-7）: ステロイド投与 + アドレナリン吸入を検討（用量はガイドライン参照。担当医が決定）'}
    return {score,severity:'ok' as const,label:'軽症（≦2）: ステロイド単回投与を検討（用量はガイドライン参照。担当医が決定）'}
  },[vals])
  return(
