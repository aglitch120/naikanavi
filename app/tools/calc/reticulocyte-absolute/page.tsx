...
export default function ReticulocyteAbsPage() {
  ...
  const result = useMemo(() => {
    ...
    if (absRet > 100000) { interpretation = `絶対値Ret = ${(absRet/10000).toFixed(1)}万/μL — 骨髄反応亢進（溶血・出血後の回復期）` }
    else if (absRet >= 25000) { interpretation = `絶対値Ret = ${(absRet/10000).toFixed(1)}万/μL — 正常範囲`; }
    else { interpretation = `絶対値Ret = ${(absRet/10000).toFixed(1)}万/μL — 産生低下（低産生型貧血）`; severity = 'wn' }
  ...
  return (
    <CalculatorLayout slug={toolDef.slug} title={toolDef.name} titleEn={toolDef.nameEn} description={toolDef.description}
      ...
      explanation={<div className="space-y-2 text-sm text-muted"><p><strong className="text-tx">計算式:</strong> 絶対値Ret = 網赤血球比率(%) × RBC(万/μL) × 100</p><p>正常: 約2.5-10万/μL。&gt;10万/μLは骨髄反応亢進、2.5万未満は産生低下。貧血時は比率(%)ではなく絶対値で評価すべき。RPIも併用。</p></div>}
