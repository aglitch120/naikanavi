  ...
  return (
    <CalculatorLayout slug={toolDef.slug} title={toolDef.name} titleEn={toolDef.nameEn} description={toolDef.description}
      ...
      result={<ResultCard severity={plt && allExcluded ? 'wn' : 'ok'}
        value={plt && allExcluded ? 'ITPに合致する除外診断チェック完了（最終診断は臨床医が判断）' : '基準を満たさない'}
        interpretation={`ITPは除外診断:\n1. 血小板 < 10万/μL: ${plt?'✓':'✗'}\n2. 二次性の原因を除外: ${allExcluded?'✓':'✗'}\n\n確定的な検査はない。末梢血塗抹・骨髄検査・抗体検査等で総合判断。`} />}
      explanation={<div className="text-sm text-muted"><p>ITPは除外診断。確定的な検査はなく、末梢血塗抹・骨髄検査等で総合判断。治療は担当医が判断。</p></div>}
