...
const items=[
  ...
  {id:'poor_chest',label:'胸部聴診所見が乏しい'},{id:'no_sputum',label:'痰がない or グラム染色で有意な菌を認めない'},{id:'wbc_low',label:'末梢血WBC 10,000/μL未満'},
]
export default function AbpcScorePage(){
  ...
  const result=useMemo(()=>{
    ...
    if(count>=4) return {count,severity:'wn' as const,label:`${count}/6項目: 非定型肺炎パターンに該当（抗菌薬選択は担当医が判断）`}
    return {count,severity:'ok' as const,label:`${count}/6項目: 細菌性（定型）肺炎パターンに該当（抗菌薬選択は担当医が判断）`}
  },[checks])
  return(
